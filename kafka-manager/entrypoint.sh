...
if [[ "${1}" == "kafka-manager" ]]; then
    ...
    declare fn=
    for fn in "logger.xml" "logback.xml"; do
        LOG_LEVEL="${LOG_LEVEL:-WARN}" \
            envsubst <"/opt/kafka-manager/conf/${fn}.tpl" >"/opt/kafka-manager/conf/${fn}"
    done
    ...
    curl -s "http://localhost:9000/clusters" \
